...
function vector_sub(vector_a, vector_b) {
	var result = [];
	for (var i = 0; i < vector_a.length; i++) {
		result.push(vector_a[i] - vector_b[i]);
	}
...
function vector_sigmoid_derivative(vector) {
	var list = [];
	for (var i = 0; i < vector.length; i++) {
		list.push(sigmoid_derivative(vector[i]));
	}
	return list;
}
		...
		link: function ($scope, $element, $attributes, $controller) {
			...
			class Trainer {
				...
				computeLayer(layerIndex, layerResults) {
					...
					for (var neuronIndex = 0; neuronIndex < neuronCount; neuronIndex++) {
						var neuronWeights = this.layerWeights[layerIndex][neuronIndex];
						var P = sigmoid(vector_dot(inputs, neuronWeights));
				...
				trainSetOnce(set) {
					 ...
					 * forward propagation
					 */
					for (var layerIndex = 0; layerIndex < this.layerWeights.length; layerIndex++) {
						this.computeLayer(layerIndex, layerResults);
					}

					/**
					 * backward propagation of errors using the chain rule
					 * gradient descent
					 */
					layerResults.push(set.b.slice()); //TODO
					console.log("layerResults", layerResults);

					console.log("R0", layerResults[0]);
					console.log("R1", layerResults[1]);
					console.log("R2", layerResults[2]);
					console.log("R3", layerResults[3]);

					console.log("W0", this.layerWeights[0]);
					console.log("W1", this.layerWeights[1]);

					var l2error = vector_sub(layerResults[3], layerResults[2]);
					var l2delta = vector_mul(l2error, vector_sigmoid_derivative(layerResults[2]));

					var l1weights = this.layerWeights[1][0];
					var l1error = vector_dot(l2delta, l1weights);
					var l1delta = vector_mul(l1error, vector_sigmoid_derivative(layerResults[1]));

					console.log("layer2", l2error, l2delta, vector_sigmoid_derivative(layerResults[2]));
					console.log("layer1", l1error, l2delta, l1weights, l1weights);

					for (var layerIndex = this.layerWeights.length - 1; layerIndex >= 0; layerIndex--) {
						for (var neuronIndex = 0; neuronIndex < this.layerWeights[layerIndex].length; neuronIndex++) {
							var neuronWeights = this.layerWeights[layerIndex][neuronIndex];
							console.log("neuronWeights", neuronWeights);
							console.log(layerResults[layerIndex + 1][neuronIndex]);
							console.log(layerResults[layerIndex + 2][neuronIndex]);
						}
					}
